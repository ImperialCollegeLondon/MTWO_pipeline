...
sys.path.append(r"E:\Raine\OneDrive - Imperial College London\IC\70007 Individual Project\MTWO_pipeline")
...
def annotate(labDir, cache_dir):
    ...
    if not isCached_aLD:
        logger.info("Annotating lab data..")
        c3dProcessed = []
        num_None = 0 
        ...
        logger.info(f"Number of Files with None data: {num_None}")
        ...
        logger.info("Annotated lab data saved.")
    else:
        logger.info("Annotated lab data loaded from cache.")
...
def resample(d):
    if len(d) != (WINDOW_SIZE * LAB_SAMPLING_RATE):
        logger.warning(f"Chunk size {len(d)} does not match expected window size {WINDOW_SIZE}. Make sure the correct cache file is used.")
        return None
...
def load_lab_data():
    ...
    logger.success("Lab data loaded successfully.")
...
# ======================================================================================================================
# Extract Walking data from LAB dataset.
# ======================================================================================================================

def annotate_walk(labDir, cache_dir):
    inputDataFiles = []

    for (dirpath, dirnames, filenames) in os.walk(labDir):
        for filename in filenames:
            # Check if the filename matches the regex and does not contain "Walking"
            if re.search("^(L|ID)[0-9]+.*.ontrackclassifier.joblib$", filename) and "Walk" in filename: # Walk
                inputDataFiles.append(os.path.join(Path(dirpath), Path(filename)))


    cacheFileName = f"annoatatedLabData_walk.joblib"
    annotatedLabData = os.path.join(cache_dir, cacheFileName)
    isCached_aLD = os.path.exists(annotatedLabData)

    # If cached, then jump directly to feature extraction
    if not isCached_aLD:
        logger.info("Annotating walking lab data..")
        c3dProcessed = []
        num_None = 0 

        for i in tqdm(range(0, len(inputDataFiles)), desc="Loading walking lab data"):
            inputData = joblib.load(inputDataFiles[i])
            if inputData is not None:
                c3dProcessed.append(inputData)
            else:
        ...
        def applyAnnotation(data, filename):
            ...
            for i in tqdm(range(1, len(data["movement"])), leave=False):
                if data["movement"][i] != isMovement:
                    segment = annotateSegment(data, startIndex, i-1, filename)
                    if isMovement == 1 and len(segment["accel"]) > 1000:
                        walkingSegments.append(segment)
                    # elif len(segment["accel"]) > 1000:
                    #     otherSegments.append(segment)
                    startIndex = i
                isMovement = data["movement"][i]
        ...
        for i in tqdm(range(0, len(c3dProcessed)), desc="Processing walking lab data"):
            applyAnnotation(c3dProcessed[i]["left"], c3dProcessed[i]["filename"] + str(" left"))
            applyAnnotation(c3dProcessed[i]["right"], c3dProcessed[i]["filename"] + str(" right"))
        ...
        logger.info("Annotated walking lab data saved.")
    else:
        logger.info("Annotated walking lab data loaded from cache.")
        data = joblib.load(annotatedLabData)
        walkingSegments = data['walkingSegments']

    return walkingSegments

def chunk_walk(walkingSegments, single_file=False):
    '''
    Preprocess: chunk lab data with overlapping windows
    '''
    window_size = int(WINDOW_SIZE * LAB_SAMPLING_RATE)  # 1500 Hz
    ...
    cacheFileName = f"LAB_ws{WINDOW_SIZE}_walk.joblib"
    checkpoint_ws = os.path.join(cache_dir, cacheFileName)
    isCached = os.path.exists(checkpoint_ws)
    ...
    if not isCached or single_file:
        ...
        # Example chunking for Movement and Other datasets
        chunkedWalk = chunkAllExamples(walkingSegments)
        # Save the checkpoint
        data = {
            'walk': chunkedWalk
        }
        ...
        logger.info("Loading Walking checkpoint...")
        data = joblib.load(checkpoint_ws)
        chunkedWalk = data['walk']
        logger.info("Checkpoint loaded.")

    return chunkedWalk

def flatten_walk(chunkedWalk, method='parallel'):
    flatWalkLab = process_with_parallel(chunkedWalk)

    if method == 'batch':
        flatWalkLab = process_in_batches(flatWalkLab, batch_size=50)
    else:
        # Delete the accel and filename columns
        flatWalkLab = [df.drop(["filename", "accel"], axis=1) for df in tqdm(flatWalkLab, desc="[info@load_lab_data.flatten] -> Flattening Walking Data", leave=False)]

    return flatWalkLab


def load_lab_walking_data():
    """
    Load the Walking class lab data. The 'load_lab_data' function does not include those walking activities.
    """
    # 1. Annotate the walking data
    walkingSegments = annotate_walk(lab_data_dir, cache_dir)

    # 2. Chunk the annotated data into overlapping windows
    chunkedWalk = chunk_walk(walkingSegments)

    # 3. Flatten the chunked data into a format suitable for training
    flatWalkLab = flatten_walk(chunkedWalk, method='parallel')

    logger.success("Walking lab data loaded successfully.")

    return flatWalkLab

if __name__ == "__main__":
    # Example usage
    joblib_path = r"E:\Raine\OneDrive - Imperial College London\IC\70007 Individual Project\Data\revised_Lab_data\L06\Baseline\L06 Walk01.c3d.ontrackclassifier.joblib"
    # 1. Annotate the lab data
